...
bool PacketBatch::CheckComplete() {
    // std::cout << "[PacketBatch] batch id: " << this->batch_id << ", batch_data_num: " << this->data_num << ", current data pkt num:" << this->data_pkts.size() << ", current fec pkt num:" << this->fec_pkts.size() << '\n';
    // FEC calculation here
    // std::cout << "data size: " << this->data_pkts.size() << ", FEC size: " << this->fec_pkts.size() << ", DATA num: " << this->data_num <<"\n";
...
Time PacketGroup::GetAvgPktInterval() {
    if(last_pkt_id_tx_0 == 0) return MicroSeconds(0);
    return (last_rcv_time_tx_0 - first_rcv_time) / (data_num + fec_num);
};
...
void PacketGroup::AddPacket(Ptr<VideoPacket> pkt, Time group_delay) {
    ...
    //     std::cout << "[PacketGroup] DUP_FEC rcvd group id: " << this->group_id << ", pkt_id_in_group: " << pkt->GetPktIdGroup() << '\n';
    // }
    ...
    if(pkt->GetRcvTime() > this->last_rcv_time_tx_0 && pkt->GetTXCount() == 0) this->last_rcv_time_tx_0 = pkt->GetRcvTime();
